...
import { CollectionTile } from "@/lib/dato/types";
  ...
  return (
    <Link href="/" className="group block bg-brand-grey rounded-lg overflow-hidden">
      <div className="p-8 relative flex flex-col items-start">
        <h3 className="font-heading-base group-hover:underline underline-offset-8">{data.title}</h3>
        <div className="w-36 h-36 translate-y-4 -translate-x-3">
          <Image src={data.image ? data.image.url : "/images/prodImg.png"} width={200} height={200} alt="test" />
        </div>
